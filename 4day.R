...
# 
#
inputData <- scan() # 빈 줄이 입력될 때까지 숫자를 입력받는다.
class(inputData)
inputData
...
n1 <- as.numeric(number1)
n2 <- as.numeric(number2)
...
num1 <- readline('Input number1 : ')
num2 <- readline('Input number2 : ')
...
#
#
# 2.2 선택 구조 : 조건에 따라 처리 방향을 결정하는 구조
#
# 선택 구조 종류
#  1. 단순 선택 구조
#  2. 양자 택일 구조
...
job.type <- 'B'
bonus <- 0
if (job.type == 'A'){bonus <- 200} else {bonus <- 100}
...
#
# 실습 문제 : 하나의 숫자를 입력 받아 짝수인지 홀수인지 출력 
#
...
cat ( 'Number :' , num4, '는', oddeven, '이다')




#
# 실습 문제 : 하나의 숫자를 입력 받아 3의 배수이면 "3의 배수"
#             5의 배수이면 "5의 배수" , 3,5의 배수가 아니면 "3,5의 배수가 아닙니다."
#             출력

num5 <- as.numeric(readline('Input Number :'))
if(num5%%3==0){
    cat(num5, '는 3의 배수입니다')
}else if(num5%%5==0){
    cat(num5, '는 5의 배수입니다')
}else{cat(num5, '는 3,5의 배수가 아닙니다')
}
